...
function App() {
  ...
  async function createPlan() {
    ...
    try {
      setPlan(normalizePlan(await postJson('/api/plan', { interpretation, ...getLocalDateTime() })))
      setCompletedTaskIds([])
  ...
  function updateInterpretationItem(category, itemId, patch) {
    setInterpretation((current) => ({ ...current, [category]: (current[category] || []).map((item) => item.id === itemId ? { ...item, ...patch } : item) }))
  }
  ...
  function confirmMove() {
    ...
    if (pendingMove.kind === 'review') {
      setInterpretation((current) => {
        const item = current[pendingMove.category]?.find((entry) => entry.id === pendingMove.itemId)
        if (!item) return current
        return { ...current, [pendingMove.category]: current[pendingMove.category].filter((entry) => entry.id !== item.id), [pendingMove.destination]: [...(current[pendingMove.destination] || []), item] }
      })
    } else {
      setPlan((current) => {
  ...
  function resetIris() {
    setBrainDump(''); setInterpretation(null); setPlan(null); setCompletedTaskIds([]); setError(null); setClarificationAnswers({}); setEditingQuestion(null)
  }

  async function submitClarifications() {
    const questions = interpretation?.questions || []
    if (questions.some((_, index) => !clarificationAnswers[index]?.trim())) {
    ...
    try {
      setInterpretation(normalizeInterpretation(await postJson('/api/interpret', { brainDump: updatedBrainDump, ...getLocalDateTime() })))
  ...
  function renderDateTime(item, onChange) {
    return <div className="editable-metadata">
      <InlineEdit type="date" value={item.date || item.due_date || ''} placeholder="add date" className="metadata-edit" onSave={(date) => onChange({ date })} ariaLabel="Edit date" />
      <InlineEdit type="time" value={item.time || ''} placeholder="add time" className="metadata-edit" onSave={(time) => onChange({ time })} ariaLabel="Edit time" />
    </div>
  }
  ...
  function renderMoveDialog() {
    ...
    return <div className="move-dialog-backdrop" role="presentation"><section className="move-dialog" role="dialog" aria-modal="true" aria-labelledby="move-dialog-title">
      <p className="eyebrow">iris</p><h2 id="move-dialog-title">you’re moving this task</h2><strong className="move-title">{pendingMove.title}</strong>
      ...
      <div className="move-actions"><button className="small-button" onClick={() => setPendingMove(null)}>cancel</button><button onClick={confirmMove}>move task</button></div>
    </section></div>
  }
  ...
  if (!plan) return <main className="app"><div className="card">
    ...
    <div className="category-list">{CATEGORY_KEYS.map(renderPreviewCategory)}</div><p className="move-hint">drag anything into another section if i got it wrong.</p>
